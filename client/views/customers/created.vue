            ...
            <article class="tile is-child box">
              ...
              <div class="content">
                <article class="tile is-child">
                  <h1 class="title has-text-centered" id="createdCustomer">Create Customer</h1>
                  <div class="block">
                    ...
                    <form id="sedForm" @submit.prevent="onSubmit">
                      ...
                      <div class="control has-addons has-addons-centered create-button">
                        <button type="submit" class="button is-info is-outlined is-medium">Create</button>
                      </div>
                    </form>
                  </div>
                </article>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        validator: null,
        selected: 'Create a New Customer',
        formData: {
          customerName: '',
          customerPhone: '',
          customerMail: '',
    ...
    computed: {
      ...mapGetters({ baseUrl: 'getBaseUrl', loader: 'getLoaderState', showModal: 'getModalVisible', token: 'getTokenKey', errorMessage: 'getErrorMessage', statusWindow: 'getStatusWindow' }),
      typeSelect () {
    ...
    methods: {
      ...
      onSubmit () {
        this.$validator.validateAll().then(() => {
          ...
          let url
          let formData
          url = this.baseUrl + 'admin/customer/create'
          this.mutateMessageState({message: 'The customer was created', status: 'success'})
          formData = {
            customerName: this.formData.customerName,
            customerPhone: this.formData.customerPhone,
            customerMail: this.formData.customerMail,
            customerAddress: this.formData.customerAddress,
            customerType: this.formData.customerType,
            customerNote: this.formData.customerNote
          }
